...
$Config = "configs/veri_r50ibn_lka_s6_nl_seed3_v1.yml"
$OutputDir = "outputs/veri_r50ibn_lka_s6_nl_amp64_seed3_v1_20260416"
...
if ($Resume -and (Test-Path -LiteralPath $FinalCheckpoint)) {
  Write-Host "Skipping s6_seed3 because $FinalCheckpoint already exists."
  exit 0
}
...
Write-Host "Starting s6_seed3: S3a structure with LKA no-freeze and lr x2"
if ($DryRun) {
...
& $Python @TrainArgs
if ($LASTEXITCODE -ne 0) {
  Write-Error "s6_seed3 failed with exit code $LASTEXITCODE."
  exit $LASTEXITCODE
}

Write-Host "s6_seed3 completed successfully."
