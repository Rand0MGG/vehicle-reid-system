param(
  [string]$EnvName = "reid_311",
  [string]$Python = "",
  [switch]$Resume,
  [switch]$DryRun,
  [switch]$SafeEval
)

$ErrorActionPreference = "Stop"

$RepoRoot = Resolve-Path (Join-Path $PSScriptRoot "..")
Set-Location $RepoRoot

if (-not $Python) {
  $CandidatePython = Join-Path $env:USERPROFILE "miniconda3\envs\$EnvName\python.exe"
  if (-not (Test-Path -LiteralPath $CandidatePython)) {
    Write-Error "Cannot find $CandidatePython. Pass -Python with the full path to the $EnvName python.exe."
    exit 1
  }
  $Python = $CandidatePython
}

$Python = (Resolve-Path -LiteralPath $Python).Path
$env:PYTHONPATH = Join-Path $RepoRoot "fastreid"

$ActualEnv = & $Python -c "import pathlib, sys; print(pathlib.Path(sys.prefix).name)"
if ($LASTEXITCODE -ne 0) {
  Write-Error "Failed to run Python at $Python."
  exit $LASTEXITCODE
}

if ($ActualEnv -ne $EnvName) {
  Write-Error "Expected conda env '$EnvName', but Python reports '$ActualEnv': $Python"
  exit 1
}

& $Python -c "import torch, yaml; import fastreid; print('env-ok')"
if ($LASTEXITCODE -ne 0) {
  Write-Error "The $EnvName environment cannot import torch, yaml, and fastreid."
  exit $LASTEXITCODE
}

$Experiments = @(
  @{
    Name = "m0: BagTricks baseline"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m0_base_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m0_base_seed3_v1_20260419"
    Entry = "fastreid/tools/train_net.py"
  },
  @{
    Name = "m1: Residual MLP x1"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m1_mlp1_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m1_mlp1_seed3_v1_20260419"
    Entry = "scripts/train_m_series_entry.py"
  },
  @{
    Name = "m3: Residual MLP x3"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m3_mlp3_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m3_mlp3_seed3_v1_20260419"
    Entry = "scripts/train_m_series_entry.py"
  },
  @{
    Name = "m6: Residual MLP x6"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m6_mlp6_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m6_mlp6_seed3_v1_20260419"
    Entry = "scripts/train_m_series_entry.py"
  },
  @{
    Name = "m12: Residual MLP x12"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m12_mlp12_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m12_mlp12_seed3_v1_20260419"
    Entry = "scripts/train_m_series_entry.py"
  },
  @{
    Name = "m24: Residual MLP x24"
    Config = "configs/m_series/veri_r50ibn_bagtricks_m24_mlp24_seed3_v1.yml"
    OutputDir = "outputs/veri_r50ibn_bagtricks_m24_mlp24_seed3_v1_20260419"
    Entry = "scripts/train_m_series_entry.py"
  }
)

foreach ($Experiment in $Experiments) {
  if (-not (Test-Path -LiteralPath $Experiment.Config)) {
    Write-Error "Missing config: $($Experiment.Config)"
    exit 1
  }

  if (-not (Test-Path -LiteralPath $Experiment.Entry)) {
    Write-Error "Missing training entry: $($Experiment.Entry)"
    exit 1
  }

  if (-not $Resume -and (Test-Path -LiteralPath $Experiment.OutputDir)) {
    $ExistingItems = Get-ChildItem -LiteralPath $Experiment.OutputDir -Force -ErrorAction SilentlyContinue
    if ($ExistingItems) {
      Write-Error "Refusing to start from scratch because $($Experiment.OutputDir) already contains files. Archive/rename it, or rerun with -Resume if this is an intentional resume."
      exit 1
    }
  }
}

foreach ($Experiment in $Experiments) {
  $FinalCheckpoint = Join-Path $Experiment.OutputDir "model_final.pth"
  if ($Resume -and (Test-Path -LiteralPath $FinalCheckpoint)) {
    Write-Host "Skipping $($Experiment.Name) because $FinalCheckpoint already exists."
    continue
  }

  $TrainArgs = @(
    $Experiment.Entry,
    "--config-file",
    $Experiment.Config,
    "--num-gpus",
    "1"
  )

  if ($Resume) {
    $TrainArgs += "--resume"
  }

  if ($SafeEval) {
    $TrainArgs += @(
      "TEST.IMS_PER_BATCH",
      "64",
      "TEST.NUM_WORKERS",
      "0",
      "TEST.PIN_MEMORY",
      "False"
    )
  }

  Write-Host "Starting $($Experiment.Name)"
  if ($DryRun) {
    Write-Host "[dry-run] $Python $($TrainArgs -join ' ')"
    continue
  }

  & $Python @TrainArgs
  if ($LASTEXITCODE -ne 0) {
    Write-Error "$($Experiment.Name) failed with exit code $LASTEXITCODE. Remaining experiments will not start."
    exit $LASTEXITCODE
  }

  Write-Host "$($Experiment.Name) completed successfully."
}

if ($DryRun) {
  Write-Host "Dry run completed. No training was started."
} else {
  Write-Host "M-series m0/m1/m3/m6/m12/m24 completed successfully."
}
