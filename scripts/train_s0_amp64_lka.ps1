...
$Experiments = @(
  @{
    Name = "s0_amp64: SBS + AMP + batch64 + NUM_INSTANCE16"
    Config = "configs/veri_r50ibn_sbs_s0_amp64_v1.yml"
  },
  @{
    Name = "s3a: SBS + LKA + Non-local on"
    Config = "configs/veri_r50ibn_lka_s3a_nl_v1.yml"
  },
  @{
    Name = "s3b: SBS + LKA + Non-local off"
...
Write-Host "s0_amp64, s3a, and s3b completed successfully."
